            ...
            <motion.div initial="hidden" animate="visible" className="max-w-xl relative z-20">
              ...
              <motion.div custom={3} variants={fadeIn} className="flex flex-col sm:flex-row gap-4 mb-10 mt-24">
                <button
                  ...
                  className="flex items-center justify-center gap-3 h-16 px-6 rounded-full bg-[#8A05FF] text-white font-black text-base shadow-lg shadow-purple-500/20 transition-all hover:scale-105 active:scale-95"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-3 h-16 px-6 rounded-full bg-[#1A1A2E] text-white font-black text-base shadow-lg shadow-black/10 transition-all hover:scale-105 active:scale-95"
                >
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {[
            ...
            <div key={i} className="p-10 rounded-[2.5rem] bg-gray-100 border border-gray-200 shadow-xl shadow-black/15 hover:shadow-2xl hover:shadow-black/25 transition-all group">
              ...
              <span className="text-xl font-black uppercase tracking-widest text-primary mb-4 block drop-shadow-md">{item.badge}</span>
      ...
      <section id="planos" className="relative py-32 bg-white/5 backdrop-blur-3xl scroll-mt-16 overflow-hidden">
        ...
        <div className="relative z-10 grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-6">
          {[
            ...
            return (
              <div key={i} className={`p-12 rounded-[3rem] border ${isSelected ? "border-primary bg-white text-black scale-105" : "border-white/10 bg-[#280c41] text-white"} flex flex-col`}>
                <h3 className="text-xl font-black uppercase tracking-widest mb-2">{plan.name}</h3>
                <div className="text-4xl font-black mb-8">{plan.price}<span className={`text-sm font-medium ${isSelected ? "text-black/70" : "text-white/70"}`}>/mês</span></div>
                <ul className="space-y-4 mb-10 flex-1">
                  {plan.features.map((f, j) => (
                    <li key={j} className={`flex items-center gap-3 text-sm font-bold ${isSelected ? "text-black" : "text-white"}`}>
                ...
                <button
                  ...
                  className={`w-full h-14 rounded-2xl font-black uppercase tracking-widest text-xs transition-all ${isSelected ? "bg-emerald-500 text-white" : "bg-white/5 border border-white/10 hover:bg-white/10"}`}
                >
                  Selecionar Plano
                </button>
              </div>
      ...
      <section id="faq" className="py-32 px-6 bg-[#280c41] text-white scroll-mt-16">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black uppercase italic tracking-tighter mb-4 text-white">Perguntas <span className="text-white">Frequentes</span></h2>
      ...
      <section className="py-12 px-6">
        <div className="max-w-7xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
            className="relative overflow-hidden rounded-[2.5rem] bg-[#280c41] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 group"
          >
            {/* Decorative circles */}
